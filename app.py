...
if not st.session_state.acceso_concedido:
    ...
    with col2:
        ...
        st.caption("Confidential: Latin American Ingredients Division")
        
        # Este es tu PIN
        pin_secreto = "LatAm2026" 
        clave_ingresada = st.text_input("Enter Access PIN:", type="password")
        
        if st.button("Unlock Simulator"):
            if clave_ingresada == pin_secreto:
...
st.title("🔬 R&D Intelligence Dashboard")
st.markdown("---")
...
with tab1:
    ...
    with col_info:
        ...
        st.markdown(f'<div class="color-box" style="background-color: {c_color}; opacity: {max(0.1, final_proc_stab/100)};">POST-PROCESSING</div>', unsafe_allow_html=True)
...
with tab2:
    ...
    with col_info2:
        ...
        st.markdown(f'<div class="color-box" style="background-color: {end_color}; opacity: {max(0.1, final_shelf_stab/100)};">END OF SHELF LIFE</div>', unsafe_allow_html=True)
        ...
        elif final_shelf_stab > 50: st.warning("⚠️ NOTICEABLE FADING")
...
st.markdown("---")
st.caption("Confidential: Robertet Ingredients Division")
